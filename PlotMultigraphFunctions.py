...
def creat_dir(path):
    ...
    inf_dir = net_dir + '/Infomap_layout'
    lou_dir = net_dir + '/Louvain_layout'
    os.makedirs(net_dir, exist_ok=True)
    os.makedirs(inf_dir, exist_ok=True)
    os.makedirs(lou_dir, exist_ok=True)
    
    return net_dir, inf_dir, lou_dir
...
def build_G(head, separator,data_path, abundance_path= []):
    ...
    if all(x in df.columns.to_list() for x in ['abundance', 'cover']):
       print('Plant abundance and cover already in data')
    else:
...
# =============================================================================
    ...
    # If not, used the input colormap
        ...
        node_colors_dict = {u: cmp_nodes.colors[edge_to_int[d[node_att]]] for u,d in G.nodes(data=True)}
